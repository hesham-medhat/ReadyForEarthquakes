...
//	}
	 ...
	 * @param fileName - file name or URL for data source
	 * 
	 * @return A HashMap of country->disaster risk reduction score.
	 */
	public static HashMap<String, Float> loadFromCSV(PApplet p, String fileName) {
		// HashMap key: country ID and data: lifeExp at birth
		HashMap<String, Float> disasterRRS = new HashMap<String, Float>();
		...
		for (int r = 1; r <= 215; r++) {
			...
			for (int i = columns.length - 1; i > 3; i--) {
				...
				if (!columns[i].equals("..")) {
					disasterRRS.put(columns[3], Float.parseFloat(columns[i]));
