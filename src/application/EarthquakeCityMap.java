 ...
 * earthquake data and disasters DRRP scores.
 * 
...
public class EarthquakeCityMap extends PApplet {
	...
	private HashMap<String, Float> disasterRRS;
	private List<Marker> countryMarkersDisasters;

	private boolean disasterMode;

	private int rectX;
	private int rectY;
	private int rectWidth = 115;
	...
	public void setup() {
		...
		countryMarkersDisasters = MapUtils.createSimpleMarkers(countries);
		map.addMarkers(countryMarkersDisasters);
		unshadeCountries();
		disasterMode = false;
		// DEBUG:System.out.println(countryMarkersDisasters.get(0).getId());
	...
	private void shadeCountries() {
		for (Marker marker : countryMarkersDisasters) {
			// Find data for country of the current marker
			String countryId = marker.getId();
			System.out.println(disasterRRS.containsKey(countryId));
			if (disasterRRS.containsKey(countryId)) {
				marker.setHidden(false);
				Float score = disasterRRS.get(countryId);
				// Encode value as brightness (values range: 40-90)
				int colorLevel = (int) map(score, 1, 5, 60, 250);
	...
	private void addKey() {
		...
		fill(255);
		textSize(12);
		text("Show/Hide DRRP", rectX + 10, rectY + 8);

		textSize(12);
		fill(0);
		text("Click to show the DRRP", rectX - 13, rectY + 35);
		text("(Disaster Risk Reduction", rectX - 13, rectY + 55);
		text("Progress) Score map.", rectX - 13, rectY + 75);
		text("Deeper blue = Safer.", rectX - 13, rectY + 95);
		text("DRRPS data is collected", rectX - 13, rectY + 120);
		text("from World Bank (2011).", rectX - 13, rectY + 140);
		text("Earthquake data is live", rectX - 13, rectY + 160);
		text("from the USGS.", rectX - 13, rectY + 180);
		text("Click on an earthquake/", rectX - 13, rectY + 205);
		text("city marker for the", rectX - 13, rectY + 225);
		text("threat circle effect.", rectX - 13, rectY + 245);

		fill(10, 10, 250);
		textSize(12);
		text("By Hesham Medhat", rectX - 13, rectY + 270);
		text("with UCSD.", rectX - 13, rectY + 290);
	}
	...
	public boolean isOverButton(int x, int y) {
		if (x >= rectX && x <= (rectWidth + rectX) && y >= rectY && y <= (rectHeight + rectY)) {
			return true;
		} else {
			return false;
		}
